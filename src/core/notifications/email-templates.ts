...
function shell(bodyHtml: string): string {
  ...
  <div style="max-width:480px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0">
    <div style="background:${BRAND};padding:16px 24px"><span style="color:#ffffff;font-weight:700;font-size:18px;letter-spacing:.2px">Klenic</span></div>
    <div style="padding:24px">${bodyHtml}</div>
    <div style="padding:16px 24px;border-top:1px solid #eef2f6;color:#94a3b8;font-size:12px">This is an automated message from Klenic. Please don't reply.</div>
  </div>
</div>`;
}
...
export function passwordResetEmail(args: {
  ...
  const subject = "Reset your Klenic password";
  const html = shell(
    `<p style="margin:0 0 12px">Hi ${escapeHtml(name)},</p>
     <p style="margin:0 0 16px">We received a request to reset your Klenic password. Click below to set a new one — this link expires in <strong>${expiresMins} minutes</strong>.</p>
...
We received a request to reset your Klenic password. Use the link below within ${expiresMins} minutes to set a new one:
