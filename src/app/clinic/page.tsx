...
export default async function ClinicDashboard() {
  ...
  const [
    ...
    db
      .select({ avgVisitValue: clinics.avgVisitValue })
      .from(clinics)
  ...
  const avgVisitValue = clinicRow[0]?.avgVisitValue ?? 3000;
  ...
  const stats = [
    ...
    { title: "Upcoming appts", value: upcoming.value, note: "Scheduled ahead", href: "/reception" },
    ...
    <div className="space-y-6">
      ...
      <Card className="border-primary/40 bg-primary/5">
        <CardHeader>
          ...
          <CardDescription>
            {recovered} return visit{recovered === 1 ? "" : "s"} driven by recall
            reminders × {new Intl.NumberFormat("en-PK").format(avgVisitValue)} PKR
            average visit value.
          </CardDescription>
        </CardHeader>
        <CardContent>
